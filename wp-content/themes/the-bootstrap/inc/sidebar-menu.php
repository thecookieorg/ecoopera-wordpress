...
class The_Bootstrap_Nav_sedebar extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth, $args ) {
		...
		$item_output	=	$args->before . '<a' . $attributes . '><span>';
		$item_output	.=	$args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
		$item_output	.=	( $args->has_children AND 1 > $depth ) ? '' : '';
		$item_output	.=	'</span></a>' . $args->after;		
 ...
 * @return	array
 */
function the_bootstrap_nav_sedebar_css_class( $classes ) {
	if ( in_array('current-menu-item', $classes ) OR in_array( 'current-menu-ancestor', $classes ) )
		$classes[]	=	'active';

	return $classes;
}
add_filter( 'sidebar_menu_css_class', 'the_bootstrap_nav_sedebar_css_class' );
